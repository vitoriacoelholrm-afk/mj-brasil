// INDICADORES DO SGQ — o que a 9.1.1 manda reter, e o veredito calculado em cima disso.
//
// Duas decisões que explicam o desenho:
//
// 1. O veredito é CALCULADO. Não existe campo "atingiu a meta" para alguém marcar. Meta, sentido
//    e resultado bastam para saber, e saber sempre do mesmo jeito. É a mesma escolha do relatório
//    de inspeção, pela mesma razão: caixinha marcada à mão é opinião, não evidência.
//
// 2. O que a planilha calculou fica guardado ao lado do que os números dizem. Quando os dois
//    discordam, o app não escolhe nenhum: mostra os dois e aponta a diferença. É o mesmo desenho
//    do `noPapel` da medição — o original nunca se perde, e quem confere vê de onde veio a dúvida.
//
// Este arquivo não sabe de empresa nenhuma. Quais são os indicadores, qual a meta de cada um e o
// que já foi apurado: tudo isso mora no perfil da empresa.

/** Para onde o número deve andar. Devolução menor é melhor; entrega no prazo, maior. */
